PRUEBAS.grupo('ZD · la demostración con clave, y quién no tiene que escribirla');
...
const ZD_CAB = ['Usuario (puede ser el que quieras)',
...
const zdJson = r => JSON.parse(r && r.getContent ? r.getContent() : r);
...
PRUEBAS.caso('🔴 sin credenciales, la demostración PIDE la clave', () => {
  ...
  const api = zdEnv(['accionDemo','validarAcceso']);
  const r = zdJson(api.accionDemo({ dispositivoId:'d-afuera' }));
...
PRUEBAS.caso('🔴 con una clave equivocada tampoco entra', () => {
  const api = zdEnv(['accionDemo','validarAcceso']);
  const r = zdJson(api.accionDemo({ pass:'probando', dispositivoId:'d-afuera-2' }));
  PRUEBAS.igual(r.ok, false, 'no abre');
...
PRUEBAS.caso('⚠️ con la clave correcta SÍ entra · el discriminador de los dos de arriba', () => {
  const api = zdEnv(['accionDemo','validarAcceso']);
  const r = zdJson(api.accionDemo({ pass:'la-clave-de-afuera', dispositivoId:'d-afuera-3' }));
...
PRUEBAS.caso('🔴 QUIEN YA TIENE SESIÓN NO ESCRIBE LA CLAVE DE LOS DE AFUERA', () => {
  ...
  const r = zdJson(api.accionDemo({ usuario:'helitec', pass:'clave-sup', dispositivoId:'d-adentro' }));
...
PRUEBAS.caso('🔒 pero una sesión INVENTADA no abre nada', () => {
  ...
  const api = zdEnv(['accionDemo','validarAcceso']);
  const r = zdJson(api.accionDemo({ usuario:'helitec', pass:'no-es-la-suya', dispositivoId:'d-falso' }));
  PRUEBAS.igual(r.ok, false, 'no abre');
...
PRUEBAS.caso('⚠️ sin `demo_pass` cargada, la demostración sigue abierta para todos', () => {
  ...
  const api = zdEnv(['accionDemo','validarAcceso'], { conClave:false });
  const r = zdJson(api.accionDemo({ dispositivoId:'d-sin-clave' }));
  PRUEBAS.falso(r.motivo === 'demo_pass' || r.motivo === 'demo_pass_mal',
...
PRUEBAS.caso('⚠️ ZD · el gate y el panel leen LA MISMA descripción', () => {
...
PRUEBAS.caso('⚠️ ZD · los textos de la demostración están en los dos idiomas (R14)', () => {
