...
la corrida completa y caen sueltos: dependen de estado que dejan archivos anteriores. Uno más que
se aisló el 2026-09-16: «el cartel no dice "Enviando…"…» de `p087-error-del-servidor.js` caía suelto
porque, sin pendientes de OTRAS colas, `offPintar` esconde la barra por el `return` temprano sin tocar
el texto viejo; el discriminador miraba el texto y no el `display`. Ahora acepta «oculta». Y al revés: un
